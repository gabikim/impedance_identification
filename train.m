...
for k = 1:9 %for each cycle percentage grouping
    ...
    [torque_delt_curr, X_curr, percentP_curr, percent_curr, torque_curr_pet, torque_curr_unpet, Hacc_curr] = init_groupData(groupsize_curr, p_size, p_end);
    count = 0;
    ...
    [percentP_curr, torque_delt_curr, X_curr, torqueU_actual, torqueP_actual, Hacc_curr] = align_cyclePercent(newSampleSize, p_size, groupsize_curr, percentP_curr, torque_delt_curr, X_curr, torqueU_actual, torqueP_actual, Hacc_curr);
    ...
    for testNum = 1:groupsize_curr 
        ...
        [torque_delt_testVec, X_testVec, percent_testVec, Hacc_testVec] = init_testVectors(testNum, torque_delt_curr, X_curr, percentP_curr, Hacc_curr);
        % calculate torque for test vector, testNum
        [torque_delt_calc, torque_delt_actual, delt_error, percent_calc, torqueU_actual_curr, torqueP_actual_curr, W_trainRHF, W_trainRKF] = calc_torque(testNum, moving_windowSize, shifted_petSize, percent_testVec, W_train, X_testVec, torque_delt_testVec, torqueU_actual, torqueP_actual, Hacc_testVec, useH);
        ...
        RKFtorque_delt_calc = smoothdata(torque_delt_calc(RKF,:) , 'gaussian', 5); 
        ...
        if plotTorqueCalc == 1 %&& count_step < 10 && count_step > 0
            valid = plot_torqueCalc(delt_error, RKFtorque_calc, RHFtorque_calc, corrRMSE_matrix, percent_calc, count_step, torque_delt_actual, RHFtorque_delt_calc, RKFtorque_delt_calc, torqueP_actual_curr, torqueU_actual_curr, currDate);
        end  
        if plotKB == 1 %&& count_step < 10 && count_step > 0
            valid = plot_KB(W_trainRHF, W_trainRKF, RKFtorque_calc, RHFtorque_calc, percent_calc, count_step, currDate);
        end
        
        %save the W matrix values to variable, 
        %columns: % gait cycle, Khh, Khk, Bhh, Bhk, K_kh, K_kk, B_hk, B_kk
        %1st 4 are for RHF, second 4 are for RKF
        KBmtx(1:size(percent_calc,2),:,count_step) = [percent_calc', W_trainRHF,  W_trainRKF];
